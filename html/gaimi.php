...
var s1=parseInt(Math.random()*10);
	...
	var yan1=document.getElementById('yzm'); 
	 yan1.value=ss;
}
	//alert(ss);
function banduan1() {
...
<body>
			...
			<h1 class="mui-title">更改密码</h1><a style="float: right; margin-top: 3%;" onclick="ab()">首页</a>
		</header>
<div class="one">
<form action="../sessions/POST/gaimiok.php" name="form1" METHOD="POST" onsubmit="return banduan3()">
<br />	<a style="margin-left:40%; font-size:30px;">姓名</a><label for="name"></label><input type="text" name="name" id="name" placeholder="请输入姓名"  autocomplete="on" required="required" />
<br />
<a style="margin-left:35%; font-size:30px;">新密码</a><label for="psw"></label><input type="password" name="psw" id="psw"  autocomplete="on" placeholder="8到16位数字字母组合" onblur="banduan1()" required="required" /><p style="font-size:38px; color:#F00;" id="demo1"></p>
<br />
<a style="margin-left:35%; font-size:30px;">手机号</a><label for="phone"></label><input type="text" name="phone" id="phone" autocomplete="on"  placeholder="请输入11位手机号" onblur="banduan2()" required="required" /><p  style="font-size:38px; color:#F00"; id="demo2"></p>
<input type="hidden" name="yzm" value="123" id="yzm" />
...
<button id="ok" name="ok"  value="确认" type="submit" style="margin-left:35%; font-size:30px;">修改</button>
